$(() => {
    ...
    function createSection(dataTable) {
        ...
        if (entity === "Fielding") {}
            ...
            for (let i = 0; i < sectionLayout.length; i++) {
                rating = sectionLayout[i];
                var data = getDataFromDataTable(dataTable, entity, rating.ColName);
                if (data) {
    ...
    function getDataFromDataTable(dataTable, entity, ratingColname) {
        if (dataTable && dataTable.data) {
            return dataTable.data.find(x => x[entity] === ratingColname);
        }
        return false;
    }
    ...
    function runMe() {
        var tables = $("table.statsplustable-zebra:visible");
